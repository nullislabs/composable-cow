 ...
 */
contract ModuleHandler is BaseConditionalOrder, OrderModule {
    constructor(Commitment.Data memory commitment) OrderModule(commitment) {}
...
contract ComposableCowDiscoveryTest is BaseComposableCowTest {
    ...
    function test_descriptor_UncommittedDoesNotAdvertise() public {
        StopLoss handler = new StopLoss(Commitment.none());
    ...
    function test_module_CommittedAdvertisesAndRoundTrips() public {
        ...
        ModuleHandler handler =
            new ModuleHandler(Commitment.Data({uris: moduleUris(), digest: digest, kind: PackageKind.SHA256}));
    ...
    function test_module_RevertsUncommittedURI() public {
        ...
        new ModuleHandler(Commitment.Data({uris: moduleUris(), digest: bytes32(0), kind: PackageKind.BZZ_MANIFEST}));
    }
    ...
    function test_module_RevertsContentAddressedWithURI() public {
        ...
        new ModuleHandler(
            Commitment.Data({uris: moduleUris(), digest: keccak256("pkg"), kind: PackageKind.BZZ_MANIFEST})
        );
    }
    ...
    function test_module_RevertsSha256WithoutURI() public {
        ...
        new ModuleHandler(Commitment.Data({uris: new string[](0), digest: keccak256("pkg"), kind: PackageKind.SHA256}));
    }
    ...
    function test_module_ContentAddressedNeedsNoURI() public {
        ModuleHandler handler = new ModuleHandler(
            Commitment.Data({uris: new string[](0), digest: keccak256("pkg"), kind: PackageKind.BZZ_MANIFEST})
        );
        assertTrue(handler.supportsInterface(type(IOrderModule).interfaceId));
    ...
    function test_module_UncommittedDoesNotAdvertise() public {
        ModuleHandler handler = new ModuleHandler(Commitment.none());
        assertFalse(handler.supportsInterface(type(IOrderModule).interfaceId));
        assertTrue(handler.supportsInterface(type(IConditionalOrderGenerator).interfaceId));
    }
    ...
    function test_module_NeedsInputSignal() public {
        ModuleHandler handler = new ModuleHandler(
            Commitment.Data({uris: new string[](0), digest: keccak256("module"), kind: PackageKind.BZZ_MANIFEST})
        );
