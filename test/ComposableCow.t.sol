...
contract ComposableCowTest is BaseComposableCowTest {
    ...
    /**
     * @dev `_buildSignature` builds the muxer payload with `abi.encodeCall`, so the
     *      selector comes from the `ISafeSignaturePayload` declaration rather than a
     *      string. Pin it to `SignatureVerifierMuxer.SAFE_SIGNATURE_MAGIC_VALUE`,
     *      which is private and so cannot be imported and compared directly.
     */
    function test_safeSignaturePayload_SelectorMatchesMuxerMagicValue() public {
        assertEq(ISafeSignaturePayload.safeSignature.selector, bytes4(0x5fd7e97d));
    }
}
